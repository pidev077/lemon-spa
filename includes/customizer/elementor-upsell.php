...
namespace LemonElementor\Includes\Customizer;
	...
	private function print_customizer_upsell() {
		...
		if ( ! isset( $plugins['elementor/elementor.php'] ) ) {
			$customizer_content .= $this->get_customizer_upsell_html(
				__( 'Install Elementor', 'lemon-main' ),
				__( 'Create a cross-site Header and Footer using Elementor & lemon theme.', 'lemon-main' ),
				wp_nonce_url(
					add_query_arg(
						[
				...
				__( 'Install &amp; Activate', 'lemon-main' ),
				get_template_directory_uri() . '/assets/images/go-pro.svg'
			);
		} elseif ( ! defined( 'ELEMENTOR_VERSION' ) ) {
			$customizer_content .= $this->get_customizer_upsell_html(
				__( 'Activate Elementor', 'lemon-main' ),
				__( 'Create a cross-site Header and Footer using Elementor & lemon theme.', 'lemon-main' ),
			...
			$customizer_content .= $this->get_customizer_upsell_html(
				__( 'Update Elementor', 'lemon-main' ),
				__( 'You need Elementor version 3.1.0 or above to create a cross-site Header and Footer.', 'lemon-main' ),
				wp_nonce_url( 'update-core.php' ),
				__( 'Update Elementor', 'lemon-main' ),
				get_template_directory_uri() . '/assets/images/go-pro.svg'
			);
		} elseif ( ! lemon_header_footer_experiment_active() ) {
			$customizer_content .= $this->get_customizer_upsell_html(
				__( 'Set Your Header &amp; Footer', 'lemon-main' ),
				__( 'Create cross-site Header and Footer using Elementor & lemon theme.', 'lemon-main' ),
				wp_nonce_url( 'admin.php?page=elementor#tab-experiments' ),
				__( 'Activate Now', 'lemon-main' ),
				get_template_directory_uri() . '/assets/images/go-pro.svg'
			);
		} else {
			$customizer_content .= $this->get_customizer_upsell_html(
